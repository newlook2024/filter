...
let data1 = data.filter(function (item1) {
  return item1.level == "oddiy"
})
let data2 = data.filter(function (item1) {
  return item1.level == "o'rta"
})
let data3 = data.filter(function(item1){
  return item1.level == "qiyin"
})
  ...
  data1.forEach(function (item, index) {
    ...
    h1.innerHTML = `
     ...
     search.addEventListener('input', function(){
      box.textContent = ''
      data1.forEach(function(a, b){
        if(a.title.toLocaleLowerCase().indexOf(search.value.toLocaleLowerCase()) > -1){
          let h1 = document.createElement('div')
          h1.innerHTML = `
          <h1>${a.title}</h1>
          `
